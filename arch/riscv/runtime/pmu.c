...
static void counter_start_one(struct pmu_hart *p, unsigned long idx)
{
	...
	if (hart_has(HART_FEAT_SSCOFPMF) && idx >= 3) {
		/*
		 * Arm the overflow interrupt: OF clear, nothing stale pending.
		 */
		uint64_t ev = mhpmevent_read((unsigned int)idx);

#if __RISCV_XLEN__ == 32
		ev |= SHIFT_U64(mhpmeventh_read((unsigned int)idx), 32);
#endif
		hw_event_write((unsigned int)idx, ev & ~BIT64(HPMEVENT_OF_BIT));
	}
		       ...
		       unsigned long flags, uint64_t value)
{
	...
	if (!counter_set_ok(base, mask))
		return SBI_ERR_INVALID_PARAM;
	/* Counter snapshots are not implemented: there is no shared memory. */
